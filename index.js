...
console.log(copy(copyOfOriginal, originalFlavors));
...
Rather than hardcoding "chocolate" into your function, pass a string as a parameter, and invoke with the argument "chocolate". This way you could 
also filter for "Vanilla", "Sherbert", etc. when those holidays roll around.
...
function getAverageWordLength(anArray){
    ...
    let words = [];
    for (let i = 0; i < anArray.length; i++){
        let splitArray = anArray[i].split(" ");
        words = words.concat(splitArray);
    }
    let avWords =  words.length /anArray.length;
    console.log(avWords);
}
